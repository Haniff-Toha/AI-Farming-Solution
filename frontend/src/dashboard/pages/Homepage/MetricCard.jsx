...
export const MetricCard = ({ title, value, unit, icon: Icon, color }) => {
  ...
  return (
    <Card className="relative overflow-hidden group">
      ...
      {/* Main content */}
      <CardContent className="relative p-4 sm:p-6 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-lg space-y-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex flex-col">
             <div className="flex items-baseline space-x-1">
                <div
                  className={`text-md sm:text-3xl font-bold bg-gradient-to-br ${gradient} bg-clip-text text-transparent`}
                >
                  {value}
                </div>
                <div
                  className={`text-md sm:text-lg font-medium bg-gradient-to-br ${gradient} bg-clip-text text-transparent`}
                >
                  {unit}
                </div>
              </div>
              <CardTitle className="text-xs sm:text-lg font-semibold text-gray-700 dark:text-gray-200">
                {title}
              </CardTitle>
              <div
          ...
          <div className="flex flex-col justify-between text-sm text-gray-500 dark:text-gray-400">
            ...
            <div className="flex items-center space-x-1">
              {/* <div
                className={`w-2 h-2 rounded-full bg-gradient-to-r ${gradient}`}
              /> */}
              <span className="text-xs mt-2">Updated just now</span>
            </div>
          </div>
        </div>
      </CardContent>
