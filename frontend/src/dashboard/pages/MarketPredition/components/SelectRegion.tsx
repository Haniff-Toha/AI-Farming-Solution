...
const SelectRegion = ({ regions, selected, setSelected }) => {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");

  const filteredRegions = useMemo(() => {
    return regions.filter((c) =>
    ...
    <div>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {selected || "Contoh: Kab. Aceh Tengah"}
        ...
        <PopoverContent
          align="start"
          sideOffset={4}
          className="w-[var(--radix-popover-trigger-width)] max-w-sm sm:max-w-md p-2"
        >
          <Input
